...
use cave_home_kubelet_rs::container_gc::{select_containers_to_remove, ContainerRecord, GcPolicy};
...
fn running(id: &str, pod: &str, name: &str, created_at: u64) -> ContainerRecord {
    ContainerRecord { running: true, ..dead(id, pod, name, created_at) }
}
...
fn nothing_removed_when_both_caps_unlimited() {
    let policy = GcPolicy { min_age_secs: 100, max_per_pod_container: -1, max_containers: -1 };
...
fn running_containers_are_never_removed() {
    let policy = GcPolicy { min_age_secs: 100, max_per_pod_container: 0, max_containers: -1 };
...
fn max_containers_recaps_units_evenly() {
    ...
    let policy = GcPolicy { min_age_secs: 100, max_per_pod_container: 5, max_containers: 4 };
    let cs = vec![
        dead("a1", "a", "x", 100),
        dead("a2", "a", "x", 200),
        dead("b1", "b", "x", 100),
...
fn max_containers_then_removes_globally_oldest() {
    ...
    let policy = GcPolicy { min_age_secs: 100, max_per_pod_container: 5, max_containers: 3 };
    let cs = vec![
        dead("c1", "a", "x", 100),
        dead("c2", "a", "x", 200),
        dead("c3", "a", "x", 300),
...
fn running_containers_do_not_count_toward_max_containers() {
    ...
    let policy = GcPolicy { min_age_secs: 100, max_per_pod_container: -1, max_containers: 2 };
    let cs = vec![
        running("r1", "a", "x", 50),
        dead("c1", "a", "x", 100),
        dead("c2", "a", "x", 200),
...
fn empty_input_removes_nothing() {
    let policy = GcPolicy { min_age_secs: 100, max_per_pod_container: 0, max_containers: 0 };
