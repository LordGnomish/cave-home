...
//!
//! cave-home is a single-binary project that avoids pulling external crates
//! where std suffices (Charter §5). The Snapcast control plane speaks JSON-RPC
//! 2.0, whose message bodies are small and well-shaped; rather than depend on
//! `serde_json`, this module hand-rolls just enough JSON to round-trip the
//! request / response / notification envelopes the engine models: objects,
//! arrays, strings, integers, and booleans.
//!
//! It is deliberately minimal — no floats, no scientific notation — because the
//! modelled message shapes only need those types. Every entry point is total:
...
pub enum Json {
    ...
    /// `true` / `false`.
    Bool(bool),
    /// A whole number. Modelled message values are all small integers.
    Int(i64),
    /// A UTF-8 string (no surrogate-pair encoding — not needed here).
    Str(String),
    /// An ordered array.
    Arr(Vec<Self>),
...
mod tests {
    ...
    fn parses_nested_and_whitespace() {
        let v = Json::parse(" { \"id\" : 8 , \"params\" : [ 1 , 2 ] } ").expect("parse");
        assert_eq!(v.get("id").and_then(Json::as_int), Some(8));
        assert_eq!(v.get("params").and_then(Json::as_arr).map(<[_]>::len), Some(2));
    }
    ...
    fn round_trips_object_deterministically() {
        let src = "{\"id\":8,\"jsonrpc\":\"2.0\",\"method\":\"Client.SetVolume\"}";
