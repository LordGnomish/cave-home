...
mod tests {
    ...
    fn cmd_has_all_phase1_subcommands() {
        let c = cmd();
        for sub in ["sysap", "devices", "channels", "light", "blind", "scene", "watch"] {
            assert!(
    ...
    fn light_requires_device_channel_value() {
        ...
        let light = c
            ...
            .unwrap();
        let args: Vec<_> = light.get_arguments().map(|a| a.get_id().as_str().to_string()).collect();
        for name in ["device", "channel", "value"] {
            assert!(args.contains(&name.to_string()), "arg {name} missing on `light`");
        }
    }
}
